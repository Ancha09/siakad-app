...
test('admin CPL report uses weighted SKS averages latest grades and mining students only', function () {
    ...
    $program->assertSee('Grafik IPK CPL Tahun Akademik 2024/2025')
        ...
        ->assertDontSee('CPL Aktif')
        ->assertDontSee('Mapping Aktif')
        ->assertDontSee('Mapping Aman')
        ->assertDontSee('Mapping Bermasalah')
        ->assertDontSee('Belum Cocok Master')
        ->assertDontSee('MK Bernilai')
...
test('CPL detail and course detail preserve filters and list only calculated students', function () {
    ...
    $detail->assertOk()
        ...
        ->assertDontSee('Mata Kuliah Bernilai')
        ->assertDontSee('<th>Mahasiswa</th>', false)
...
test('admin can download filtered CPL reports as PDF and Excel', function () {
    ...
    expect($pdfHtml)
        ...
        ->not->toContain('Mapping Aktif')
        ->not->toContain('Mapping Aman')
        ->not->toContain('Mapping Bermasalah')
        ->not->toContain('Belum Cocok Master')
