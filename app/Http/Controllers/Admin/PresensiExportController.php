...
class PresensiExportController extends Controller
{
    ...
    private function data(Request $request): Collection
    {
        ...
        return Presensi::with([
            ...
            ->when($request->filled('prodi_id'), fn (Builder $q) => $q->whereHas('krs.mahasiswa', fn (Builder $m) => $m->where('prodi_id', $request->prodi_id)))
            ->when($request->filled('kelas_id'), fn (Builder $q) => $q->whereHas('krs.mahasiswa', fn (Builder $m) => $m->where('kelas_id', $request->kelas_id)))
            ->when($request->filled('dosen_id'), fn (Builder $q) => $q->whereHas('krs.jadwal', fn (Builder $j) => $j->where('dosen_id', $request->dosen_id)))
            ->when($request->filled('mata_kuliah_id'), fn (Builder $q) => $q->whereHas('krs.jadwal', fn (Builder $j) => $j->where('mata_kuliah_id', $request->mata_kuliah_id)))
