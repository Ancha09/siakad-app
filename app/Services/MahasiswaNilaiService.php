...
class MahasiswaNilaiService
{
    ...
    {
        $khs = Khs::with(['krs.jadwal.mataKuliah', 'krs.kuesioner'])
    ...
    {
        $totalSks = $khs->sum(fn (Khs $item) => (int) ($item->krs?->jadwal?->mataKuliah?->sks ?? 0));
        ...
        $totalMutu = $khs->sum(function (Khs $item) {
            $sks = (int) ($item->krs?->jadwal?->mataKuliah?->sks ?? 0);
